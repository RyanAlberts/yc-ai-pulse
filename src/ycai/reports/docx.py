...
def build_memo(
    ...
    *,
    output_path: Path,
    pov_essays: dict[str, dict] | None = None,
    b2b_topology: dict | None = None,
    external_profiles: dict | None = None,
) -> Path:
    """Generate the narrative .docx memo. Layer 2 audit runs before write.

    Optional substantive-analysis inputs (produced by ``ycai enrich-context``):

    - ``pov_essays``: dict keyed by figure_key ('andreessen' / 'dalio' /
      'acemoglu') with substantive 200-300 word paragraphs replacing the
      templated three-POV intro.
    - ``b2b_topology``: bottom-up clusters of the B2B SaaS sub-cohort,
      replacing the YC-taxonomy table with categories that come from
      what these companies actually do.
    - ``external_profiles``: HN/Reddit/GitHub signals per company, surfaced
      in the traction section and the spotlights.

    When any of these is missing the memo falls back to the v0.2.0
    templated rendering for that section.
    """
    pov_essays = pov_essays or {}
    b2b_topology_data = b2b_topology or {}
    external_profiles = external_profiles or {}
    ...
    if pov_essays and len(pov_essays) >= 2:
        # Substantive path: render each figure's actual essay, generated by
        # the LLM with the cohort + HN/Reddit signals as context. This is
        # the read the user pushed back on the templated version for.
        _add_para(
            doc,
            "What this batch implies for capital allocation depends on whose frame you adopt. "
            "The three paragraphs below were generated by reading the entire high-confidence "
            "cohort (taglines, capabilities, traction, public discussion on HN and Reddit) "
            "from each named figure's published worldview. Each paragraph cites specific "
            "companies by slug; the memo does not pick a winner.",
            color=MUTED,
            size=10,
        )
        for key in ("andreessen", "dalio", "acemoglu"):
            essay = pov_essays.get(key)
            if not essay:
                continue
            heading = f"{essay.get('figure_name', key)} — {essay.get('affiliation', '')}"
            sub = doc.add_heading(heading, level=2)
            if sub.runs:
                sub.runs[0].font.color.rgb = ACCENT
            paragraph_text = essay.get("paragraph", "")
            _add_para(doc, paragraph_text)
            cited = essay.get("cited_slugs", [])
            if cited:
                _add_para(
                    doc,
                    "Cited: " + ", ".join(f"`{s}`" for s in cited),
                    italic=True,
                    color=MUTED,
                    size=9,
                )
            # POV-essay text is per-figure substantive prose. We treat it as
            # quoted_facts for Layer 2: forbidden-phrase scanned, but not
            # drift-checked against aggregate counters (the figure-level
            # arguments naturally cite per-company facts).
            quoted_facts.append(paragraph_text)
    else:
        # Fallback to templated prose when ``ycai enrich-context`` hasn't run.
        ...
        intro_paragraph = (
            ...
            "pick a winner; it equips the reader to. (Run `ycai enrich-context <run>` to replace "
            "this templated paragraph with three substantive multi-paragraph essays grounded in "
            "named cohort companies.)"
        )
    ...
    # 7. Inside B2B SaaS — bottom-up topology (when available) or YC fallback
    if b2b_topology_data and b2b_topology_data.get("clusters"):
        _add_heading(doc, "Inside B2B SaaS", level=1)
        clusters = b2b_topology_data.get("clusters", [])
        unclustered = b2b_topology_data.get("unclustered", [])
        total_b2b = b2b_topology_data.get("total_b2b", sum(b2b_subs.values()))
        intro_text = (
            f"B2B SaaS is the largest top-level bucket but the laziest description. "
            f"Of {total_b2b} B2B SaaS companies in the cohort, the topology below was generated "
            "bottom-up: a Sonnet pass over every B2B SaaS company's tagline, capabilities, "
            "traction, and public discussion (HN/Reddit). The clusters that emerged are not the "
            "YC dropdown labels (Sales, Operations, Engineering); they are the patterns the "
            "cohort actually exhibits."
        )
        _add_para(doc, intro_text)
        aggregate_prose.append(intro_text)
        for cluster in clusters:
            cluster_heading = f'{cluster.get("name")} ({len(cluster.get("member_slugs", []))} companies)'
            sub = doc.add_heading(cluster_heading, level=2)
            if sub.runs:
                sub.runs[0].font.color.rgb = ACCENT
            thesis_text = cluster.get("thesis", "")
            _add_para(doc, thesis_text, italic=False, size=11)
            members_text = ", ".join(f"`{s}`" for s in cluster.get("member_slugs", []))
            _add_para(doc, "Members: " + members_text, italic=True, size=10, color=MUTED)
            quoted_facts.append(thesis_text)
        if unclustered:
            _add_heading(doc, "Unclustered", level=2)
            _add_para(
                doc,
                f"{len(unclustered)} B2B SaaS companies didn't fit a meaningful cluster: "
                + ", ".join(f"`{s}`" for s in unclustered),
                italic=True,
                size=10,
                color=MUTED,
            )
    elif b2b_subs:
        # Fallback when ``ycai enrich-context`` hasn't run: YC-taxonomy table.
        _add_heading(doc, "Inside B2B SaaS", level=1)
        b2b_total = sum(b2b_subs.values())
        sub_text = (
            f"B2B SaaS is the largest top-level bucket. Of {b2b_total} B2B SaaS companies in "
            "the cohort, the YC-supplied sub-industry breakdown is below. (Pure passthrough — "
            "not LLM-derived — so the breakdown can't drift from the underlying classifications. "
            "Run `ycai enrich-context <run>` to replace this with a bottom-up topology grounded "
            "in what these companies actually do, with categories that come from the data rather "
            "than the YC dropdown.)"
        )
